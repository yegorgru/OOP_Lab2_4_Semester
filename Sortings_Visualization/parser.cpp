...
int parser::prior(char& in)//возвращает приоритет операции
{
    if(in == '+' || in == '-')//если + или -,
...
bool parser::is_digit(char& in)//говорит о том, число это или нет
{
    return (in >= '0' && in <= '9');
}
bool parser::is_var(char& in)//переменная ли
{
    return ((in >= 'a' && in <= 'z') ||
            (in >= 'A' && in <= 'Z'));
}

bool parser::is_oper(char& in)
{
    return (in == '+' || in == '-' ||
            in == '*' || in == '/' ||
            in == '^');
 ...
 queue<char> parser::to_revpol(const QString &input)
{
    ...
    for(int j = 0; j < in.size(); j++)
    {
        ...
        else if(is_digit(i))//если сейчас число
        {
            ...
            { stk.push('^'); gen.push(';'); clbr = false;}//это нужно, чтобы не писать знак степени после переменных и скобко, например, x2 == x^2 или (1+2)2 == (1+2)^2
            gen.push(i);
        }
        else if(is_var(i))//если сейчас переменная
        {
            ...
            { stk.push('*'); gen.push(';'); }//эта хрень нужна чтобы не ставить знак умножения перед переменной при вводе 5x == 5*x, например
            gen.push(i);
        }
...
double parser::calc(queue<char> in)
{
